$(document).ready(function () {
  ...
  const totalDynamicCols = 17; // Dynamic columns: 1 = DEFAULT, 2 = AMZ, ..., 17 = FCRY
  ...
  function addMainRow(index) {
    ...
    // Second column: Dynamic "Default" column (data-col = 1)
    ...
    return options
      .map(opt => `
        <li class="d-flex justify-content-between">
          ...
          <span class="mandatory-text disabled">Mandatory</span>
        </li>
      `)
      .join("");
  }
  ...
  function fillExistingEmptyCell(rowIndex, colIndex) {
    ...
    for (let subRow of subRows) {
      // Use nth-child(colIndex+1) because in sub rows, cell index = data-col + fixed offset (1)
              ...
              columns += `<td>
                  <div class="dropdown-container">
                      ...
                      <button class="btn btn-sm btn-danger remove-row">
                          <img src="./assets/images/delete.svg" alt="Remove" />
                      </button>
                  </div>
              </td>`;
          } else {
  ...
  $(document).on("change", ".mandatory-checkbox", function () {
    ...
    if ($(this).is(":checked")) {
      ...
      // Disable all checkboxes in the same dropdown-menu except the current one
      $(this)
    ...
    // Call our new function to enforce single selection within this dropdown.
    setTimeout(function() {
      enforceSingleSelection($(this).closest("ul.dropdown-menu"));
    }.bind(this), 100);
  ...
  $(document).on("click", ".mandatory-text", function (e) {
      ...
      if (!$(this).hasClass("active")) {
        // Toggle on: add active class, append asterisk and disable others
        ...
        // Toggle off: remove active class, remove asterisk and enable all
  ...
  // --- New Function: Enforce Single Selection in Dropdown ---
  // This function ensures that if one checkbox in a dropdown is checked,
  // all other checkboxes in that same dropdown are disabled.
  function enforceSingleSelection($dropdownMenu) {
    let $checkboxes = $dropdownMenu.find(".mandatory-checkbox");
  ...
  // --- Disable duplicate selections across same column ---
  function updateDisabledOptionsForColumn(colIndex) {
    ...
    options.forEach(function (opt) {
      ...
      $("#table-body tr").each(function () {
        // For both main and sub rows, dynamic cells are in order.
  ...
  // --- Update Parent Row Indicator ("or") ---
  // For main rows, dynamic cell is at index = colIndex (since cell 0 is the row number)
  ...
  // --- New Function: Hide mandatory-text for Default column (data-col = 1) ---
  function hideDefaultMandatoryText() {
      // Find all table cells that contain an add-row button with data-col = 1
      $("#table-body td").each(function () {
      let $cell = $(this);
      let $btn = $cell.find(".add-row");
      if ($btn.length && $btn.data("col") == 1) {
          // Hide any mandatory-text in this cell
          $cell.find(".mandatory-text").hide();
      }
      });
  }

  // Call this function after each update.
  setTimeout(hideDefaultMandatoryText, 100);
